...
class Followers(private val users: Users, private val relationships: Relationships, private val authService: AuthService) {
    fun exec(request: Request): Response {
        ...
        authService.validateToken(request.token, users.get(request.requesterUserName))
        return Response(followers)
    }
    data class Request(val requesterUserName: String, val fromUser: String, val token: SessionToken)
